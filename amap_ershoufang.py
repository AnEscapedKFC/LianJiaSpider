...
def get_geocode(xiaoqu, address, api_key, base_url, max_retries=3, base_sleep=0.5):
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            params = {
                "key": api_key,
                "address": f"上海市{xiaoqu}区{address}小区",  # 拼接完整地址
            }
...
if __name__ == "__main__":
    Area_Name = input("请输入区名(中文)：").strip()  # 需要区名提高精度
    input_file = input("请输入二手房表格文件路径(*.csv)：").strip()  # 修改为实际文件名
    ...
    # 清洗单价列（假设单价列格式为"50000元/平米"或"50,000"）
    df['单价'] = df['单价'].astype(str).str.replace('元/平米', '', regex=False)
    df['单价'] = df['单价'].str.replace(',', '', regex=False).astype(float)

    # 3. 统计小区房源数量和均价
    grouped = df.groupby('小区名称', as_index=False).agg(
        房源数量=('单价', 'count'),
        平均单价=('单价', 'mean')
    )
    # 4. 配置高德API参数
    ...
    for idx, row in grouped.iterrows():
        lng, lat = get_geocode(Area_Name, row['小区名称'], api_key, base_url, max_retries=max_retries, base_sleep=0.5)
        grouped.at[idx, '经度'] = lng
        grouped.at[idx, '纬度'] = lat
